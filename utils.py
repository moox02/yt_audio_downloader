...
def get_video_title_from_youtube_url(url):
    yt = YouTube(url)
    return yt.title
...
def get_audio_from_youtube_url(url):
    ...
    if not os.path.exists('data'):
        os.makedirs('data')
    return yt.streams.filter(only_audio=True).first().download(filename=os.path.join('data',get_video_title_from_youtube_url(url)+'.mp3'))
...
def transcribe_youtube_video(_model, url):
    ...
    result = _model.transcribe(os.path.join('data',get_video_title_from_youtube_url(url)+'.mp3'), **options.__dict__)
